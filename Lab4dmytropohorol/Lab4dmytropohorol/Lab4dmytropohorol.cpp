...
int main()
{
	// 1) Create a Taxi with constructor
	...
	// 3) Overloaded file output
	std::ofstream fout("taxi_data.txt");
	if (fout.is_open()) {
		fout << t2;
		fout.close();
		std::cout << "[MAIN] Wrote t2 to 'taxi_data.txt'.\n";
	}

	// 4) Overloaded file input
	Taxi t3;
	std::ifstream fin("taxi_data.txt");
	if (fin.is_open()) {
		fin >> t3;
		fin.close();
		std::cout << "[MAIN] Read t3 from 'taxi_data.txt'.\n";
	}
	std::cout << "Now t3 is:\n";
